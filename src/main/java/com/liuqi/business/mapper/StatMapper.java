...
public interface StatMapper {
    ...
    @Select("<script>" +
            "select t.send_date,t.currency_id,c.name as currencyName,sum(quantity) as quantity from t_lock_release t join t_currency c on t.currency_id =c.id  " +
            " group by currency_id,send_date order by send_date desc" +
            "</script>")
    List<ReleaseStatDto> releaseStat();
